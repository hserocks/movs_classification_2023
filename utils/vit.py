...
def prepare_data_vit(images_folder_path):
    ...
    # Split the dataset
    train_dataset_adj, test_dataset_adj = \
        random_split(dataset_norm, [train_size, test_size])
    ...
    train_loader_adj = DataLoader(
        train_dataset_adj,
        batch_size=32,
        shuffle=False,
        num_workers=num_workers)
    test_loader_adj = DataLoader(
        test_dataset_adj,
        batch_size=32,
        shuffle=False,
        num_workers=num_workers)
...
def train_vit(device, no_of_classes, train_loader, test_loader):
    ...
    model = timm.create_model(
        ...
        pretrained=True)  # to check other models as well
...
def test_vit(no_of_classes, test_loader):
    model = timm.create_model(
        'vit_base_patch32_clip_224.openai_ft_in1k',
        pretrained=True)  # to check others models as well
